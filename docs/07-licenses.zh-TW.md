...
> **最後查核日：2026-08-28。** 下面每一條在那一天都對著上游的 LICENSE 檔、模型卡
> 或儲存庫重新核對過。授權會變、模型卡會被編輯，所以一份沒有日期的授權文件是一份
> 你無法據以行動的文件——包括這一份，只要它夠舊。
...
Sync Labs 賣的是託管 API。他們沒有提供商用授權的權重——
商用路徑是他們的服務，不是這些檔案的授權。
...
頂層 import、在 `__init__` 裡無條件建構，所以每次執行權重都會載入。
偵測器本身只在**第一幀**被呼叫——`if last_lmk is None`，之後管線改用地標追蹤。
這修正了機制的描述，但完全不影響授權結論：檔案照樣被打包、照樣被載入。**沒有接上任何替代的偵測器。** 那些模型被打包在一個帶著 Apache-2.0 LICENSE 的權重儲存庫裡，而 Ant Group 並不處在能對它們授予這個授權的位置。
